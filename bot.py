...
MODEL_KW = [
    "model", "модель", "onlyfans", "of", "only fans", "blogger", "блогер",
    "блогерша", "content", "creator", "influencer", "инфлюенсер",
...
async def do_market_search(
    ...
    found        = 0
    seen_slugs   = set()
    owner_buckets = {}     # uid -> bucket (для режима без категории)
    owner_seen    = {}     # uid -> count (для дедупликации)
    last_upd      = 0.0
    START_TIME    = asyncio.get_event_loop().time()
    HARD_LIMIT    = 25.0

    # Шаг 1: быстрый флор для фильтрации по категории
    async def get_floors_fast(ids):
        BATCH = 40
        valid = []
    ...
    async def process_item(item, floor, has_cat):
        nonlocal found
        ...
        if has_cat:
            ...
            if oid:
                cnt = owner_seen.get(oid, 0)
                owner_seen[oid] = cnt + 1
    ...
    try:
        ...
        valid_gids = await get_floors_fast(gift_ids)
        if not valid_gids:
            is_searching = False
            return 0
        ...
        ids_to_scan = [gid for gid, _ in valid_gids]

        await status_msg.edit_text(
            "<b>Найдено коллекций: " + str(len(ids_to_scan)) + ". Сканирую...</b>",
            reply_markup=stop_kb()
        )

        # Параллельный скан всех коллекций одновременно — первая страница каждой
        BATCH = 40
        for i in range(0, len(ids_to_scan), BATCH):
            if not is_searching or found >= max_results:
                break
            if asyncio.get_event_loop().time() - START_TIME > HARD_LIMIT:
                break
            batch   = ids_to_scan[i:i+BATCH]
            results = await asyncio.gather(*[fetch_market_page(gid, "", limit=100) for gid in batch])
            tasks   = []
            for gid, (items, _) in zip(batch, results):
                floor = floor_map.get(gid, 0)
                for item in items:
                    tasks.append(process_item(item, floor, has_cat))
...
async def do_profile_search(
    ...
    is_searching = True
    found        = 0
    # uid -> кол-во лотов на маркете
    owner_market_count = {}
    # uid -> (owner_obj, username, name)
    seen_owners  = {}
    last_upd     = 0.0
    START_TIME   = asyncio.get_event_loop().time()
    HARD_LIMIT   = 60.0  # увеличили лимит для профилей

    # Шаг 1: собираем всех владельцев с маркета + считаем сколько у них лотов
    ...
    # Шаг 2: для каждого владельца проверяем профиль
        ...
        try:
            ...
            for gift in (getattr(result, "gifts", None) or []):
                ...
                if nft_url:
                    ...
                    title = getattr(inner, "title", None) or getattr(gift, "title", "?")
                    num   = getattr(gift, "num", "?")
                    items.append({"title": title, "num": num, "nft_url": nft_url, "price": None})
            return items
        except Exception as e:
    ...
    async def check_one(uid, owner_obj, username, name):
        ...
        # Скрытые = в профиле но не на рынке
        ...
        # Показываем только скрытые (не выставленные) гифты
        hidden_items = nft_gifts  # все гифты профиля
        cache_owner(uid, owner_obj, username, name, profile_url, hidden_items)
        kb  = owner_card_kb(username, profile_url, uid)
        txt = (
            "<b>Владелец: " + owner_str + "\n"
            "В профиле: " + str(profile_cnt) + " | На рынке: " + str(market_cnt) + " | Скрытых: " + str(hidden_cnt) + "</b>"
        )
        try:
            await status_msg.bot.send_message(
                chat_id=status_msg.chat.id, text=txt,
    ...
    try:
        ...
        await status_msg.edit_text(
            "<b>Найдено продавцов: " + str(total) + ". Проверяю профили...</b>",
            reply_markup=stop_kb()
        )

        PBATCH  = 10  # меньший батч чтобы не флудить
        checked = 0
        for i in range(0, len(all_owners), PBATCH):
            ...
            await asyncio.gather(*[check_one(uid, obj, un, nm) for uid, (obj, un, nm) in batch])
            checked += len(batch)
            now = asyncio.get_event_loop().time()
            if now - last_upd > 2.0:
                try:
...
async def run_market(cb, cat=None, girls=False, model=False, ids=None, col_name=None):
    global is_searching
    if is_searching:
    ...
    found = await do_market_search(status, ids, cat=cat, girls_only=girls, model_only=model, boost=boost, min_gifts=mn, max_gifts=mx)
    try:
...
async def run_profile(cb, girls=False, model=False, ids=None):
    global is_searching
    if is_searching:
        await cb.answer("Поиск уже идет!", show_alert=True)
        return
    ...
    status = await cb.message.answer(txt, parse_mode="HTML", reply_markup=stop_kb())
    found = await do_profile_search(status, ids, girls_only=girls, model_only=model, min_gifts=mn, max_gifts=mx)
    try:
        await status.edit_text(
...
async def cb_mdlcol(cb: CallbackQuery):
    idx  = int(cb.data[7:])
    ...
    await run_market(cb, ids=[col_id], model=True, col_name=col_name)
