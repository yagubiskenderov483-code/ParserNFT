...
def is_girl(owner, username=None, name=None, strict=False):
    ...
    if strict:
        if has_girl_name or has_girl_kw:
            return score >= 3
        return score >= 4
    # Обычный режим для «только девушки»: имя / окончание / сигналы, без «мутных»
    return score >= 2
...
def is_weird_cringe(owner, username=None, name=None):
    ...
    if any(k in tokens or k in low for k in anime_kw):
        score += 2
    # chan/kun/sama — только как суффикс ника
    if re.search(r"(chan|kun|sama|nya)\b", low):
        score += 1
...
def who_pass(owner, username, name, who):
    if who == "girls":
        return is_girl(owner, username, name)
    if who in ("weird", "cringe", "pozor"):
...
def price_in_cat(price, cat):
    ...
    if price <= 0:
        return False
    CATS = {
        "cheap":   (1,      2000),      # до 2000
        "mid":     (2001,   5000),      # 2001–5000
        "hard":    (5001,   20000),     # 5001–20000
        "ultra":   (20001,  100000),    # 20001–100000
        "extreme": (100001, 10**12),    # от 100001
    }
    c = CATS.get(cat)
    if not c:
        return True
...
def mark_seen(uid=None, username=None, nft_url=None):
    ...
    try:
        ...
        pass
    # простая защита от раздувания RAM
    if len(SEEN_GLOBAL) > SEEN_GLOBAL_MAX:
        # выкидываем ~половину (set не упорядочен — лучше чем вечный рост)
...
def item_matches_collections(item, allowed_gids=None, allowed_titles=None):
    ...
    if slug and norms:
        if nb and nb in norms:
            return True
        # slug есть, но коллекция другая — мимо (даже если gift_id подставили)
        if nb:
            return False

    # из БД без проверяемого slug — не доверяем одному title/gift_id
...
async def do_profile_search(status_msg, gift_ids, cat=None, girls_only=False,
    ...
    async def check_one(info):
        ...
        try:
            ...
            # mark только выданный аккаунт + первый NFT (не жжём остальные)
    ...
    try:
        ...
        if is_searching and found[0] < max_results:
            ...
            try:
                await status_msg.edit_text(
                    "<b>Профиль / " + esc(who_label(who)) + ":</b> БД ("
                    + str(st.get("users", 0)) + " юзеров) · лимит "
                    + str(max_results) + "...",
                    parse_mode="HTML", reply_markup=stop_kb()
                )
            except Exception:
                pass
            cands = random_users_from_db(limit=max(400, max_results * 20))
        ...
        if is_searching and found[0] < max_results:
            ...
            for ch in NFT_SCAN_CHATS[:8]:
                ...
                for (u_obj, uid) in res:
                    ...
                    name = (fn + " " + ln).strip()
                    # позор — не режем тут, enrich в check_one
        ...
        # если недобор до лимита — второй проход (скрытые NFT, не строго 0 на маркете)
        if is_searching and found[0] < max_results:
            try:
                await status_msg.edit_text(
            ...
            more = random_users_from_db(limit=max(300, max_results * 15))
            random.shuffle(more)
            for h in more:
                if not is_searching or found[0] >= max_results:
                    break
...
async def do_market_search(status_msg, gift_ids, cat=None, girls_only=False,
    ...
    async def try_send(item, use_global_seen=True, enforce_cap=False):
        ...
        slug = gift_slug_of(nft_url)
        gid = item.get("gift_id")
        ok_key = ("id:" + str(int(oid))) if oid else ("u:" + str(uname).lstrip("@").lower())
        ...
        try:
            ...
            # mark только после успешной отправки
            mark_seen(oid, uname, nft_url)
            try:
            ...
            async with lock:
                ...
                if gid is not None and col_counts.get(gid, 0) > 0:
                    col_counts[gid] = col_counts.get(gid, 1) - 1
            return False
    ...
    async def accept_who(item):
        ...
        try:
            ...
            if res is None:
                # flood/unknown — не считаем окончательным отказом бюджета
                weird_checks[0] = max(0, weird_checks[0] - 1)
                return False
            return bool(res)
        except Exception:
    ...
    try:
        ...
        if is_searching and found[0] < max_results and leftover:
...
async def do_model_search(status_msg, gift_ids, girls_only=False,
    ...
    async def fetch_col(gid):
        ...
        out = []
        offset = ""
        for _ in range(4 if single else 3):
            if not is_searching:
                break
            try:
                items, nxt = await fetch_market_page(gid, offset, limit=100, newest=True)
    ...
    async def emit_pool(pool, use_global_seen=True, enforce_cap=False):
        for it in diversify(pool):
            ...
            if weird_mode and not await accept_who(it):
                continue
            if (not weird_mode) and who == "girls" and not who_pass(it.get("owner"), it.get("username"), it.get("name"), who):
                continue
    ...
    try:
        ...
        for i in range(0, len(gids), PARALLEL):
            ...
            for part in parts:
                if isinstance(part, list):
                    pool.extend(part)
            if pool and found[0] < max_results and (single or len(pool) >= max_results * 2):
                batch = pool
                pool = []
                await emit_pool(batch, use_global_seen=True, enforce_cap=(not single))
            await progress()

        if pool and found[0] < max_results:
            await emit_pool(pool, use_global_seen=True, enforce_cap=(not single))
        ...
        # добор из БД по exact slug
        if is_searching and found[0] < max_results and allowed_titles:
            await progress("добор из БД...")
            need = max_results - found[0]
            hits = []
            titles_q = allowed_titles if single else random.sample(
                allowed_titles, min(len(allowed_titles), 60)
            )
            for t in titles_q:
                part = random_from_pricenft_db(limit=max(need * 8, 80), model=t, exact=True)
                for h in part:
                    h = dict(h)
                    h["title"] = t
                    h["model"] = t
                    ...
                    hits.append(h)
            random.shuffle(hits)
            await emit_pool(hits, use_global_seen=True, enforce_cap=False)

        # повтор без global seen — по ВСЕМ коллекциям, пока не набрали лимит
        if is_searching and found[0] < max_results:
            await progress("повтор без антидубля...")
            soft = []
            for g in gids:
                if not is_searching or found[0] >= max_results:
                    break
                soft.extend(await fetch_col(g))
                if len(soft) >= max_results * 4:
                    await emit_pool(soft, use_global_seen=False, enforce_cap=False)
                    soft = []
            if soft and found[0] < max_results:
                await emit_pool(soft, use_global_seen=False, enforce_cap=False)
...
async def do_profile_model_search(status_msg, gift_ids, girls_only=False,
                                  max_results=30, region="any", who=None):
    """
    Модели по профилю: скрытые NFT нужной коллекции, 0 на маркете.
    Параллельные воркеры + позор через enrich + выдача до max_results.
    """
    global is_searching
    is_searching = True
    who = who or ("girls" if girls_only else "all")
    ...
    async def accept_who(info):
        owner_obj = info.get("owner")
        username = info.get("username")
        name = info.get("name")
        if not weird_mode:
            return who_pass(owner_obj, username, name, who)
        if is_weird_cringe(owner_obj, username, name):
            return True
        async with weird_lock:
            if weird_checks[0] >= WEIRD_ENRICH_LIMIT:
    ...
    async def check_user(info):
        ...
        if (not ignore_global[0]) and is_owner_seen(uid, username):
            return
        if not await accept_who(info):
            return
        owner_obj = info.get("owner") or owner_obj
        ...
        matched = [
            g for g in saved
            if g.get("nft_url") and gift_ok(g) and (
                ignore_global[0] or not is_gift_seen(g.get("nft_url"))
            )
        ]
        if not matched:
            return
        ok_key = ("id:" + str(int(uid))) if uid else ("u:" + str(username).lower())
        async with lock:
            if found[0] >= max_results or ok_key in seen_owners:
                return
            seen_owners.add(ok_key)
            found[0] += 1
        profile_url = info.get("profile_url") or (("https://t.me/" + username) if username else None)
        ...
        nft_url = matched[0].get("nft_url")
        txt = (
            "<b>" + owner_s + "</b>"
            + (("\n👤 @" + esc(username)) if username else "")
            + "\n🎨 " + esc(str(show))
            + ('\n<a href="' + nft_url + '">' + esc(str(show)) + "</a>" if nft_url else "")
            + "\nNFT в профиле (0 на маркете): " + str(len(matched))
        )
        kb = model_card_kb(username, profile_url, uid or 0, nft_url, nft_count=len(matched))
        try:
            await status_msg.bot.send_message(
                chat_id=status_msg.chat.id, text=txt,
                parse_mode="HTML", reply_markup=kb,
            ...
            logger.warning("profile model send: %s", e)
            async with lock:
                found[0] = max(0, found[0] - 1)
                seen_owners.discard(ok_key)
    ...
    async def worker():
        while True:
            if not is_searching or found[0] >= max_results:
                return
            try:
                info = await asyncio.wait_for(q.get(), timeout=0.35)
            except asyncio.TimeoutError:
                if feeder_done.is_set() and q.empty():
                    return
                continue
            try:
    ...
    try:
        if not allowed_gids:
            ...
            return 0
        await status_msg.edit_text(
            "<b>Модель/профиль: " + esc(str(label)) + " / " + esc(who_label(who)) + "\n"
        ...
        for t in titles_q:
            cands.extend(random_from_pricenft_db(limit=100, model=t, exact=True))
        random.shuffle(cands)
        for h in cands:
            if not is_searching or found[0] >= max_results:
                break
        ...
        try:
            ...
            for w in workers:
                w.cancel()
        # soft: реально без global seen
        if is_searching and found[0] < max_results:
            ignore_global[0] = True
            feeder_done.clear()
            workers2 = [asyncio.create_task(worker()) for _ in range(6)]
            more = []
            for t in titles_q[:30]:
                more.extend(random_from_pricenft_db(limit=50, model=t, exact=True))
            random.shuffle(more)
            for h in more:
                if not is_searching or found[0] >= max_results:
                    break
            ...
            feeder_done.set()
            try:
                await asyncio.wait_for(asyncio.gather(*workers2, return_exceptions=True), timeout=90)
            except asyncio.TimeoutError:
                for w in workers2:
                    w.cancel()
...
async def _start_profile(cb, cat, who="all"):
    ...
    found = await do_profile_search(status, ids, cat=cat, who=who,
...
async def _start_model(cb, who="all", single_gid=None, search_type="market",
    ...
    try:
        ...
        if search_type == "profile":
            found = await do_profile_model_search(status, ids, who=who,
                                                  max_results=lim, region=reg)
        else:
            found = await do_model_search(status, ids, who=who,
                                          max_results=lim, region=reg)
        try:
            done = "<b>✅ Поиск закончен\nМодели / " + who_l + " / " + type_l + "\nНайдено: " + str(found) + "</b>"
            await status.edit_text(done, parse_mode="HTML", reply_markup=menu_kb())
