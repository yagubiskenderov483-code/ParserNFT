...
def region_match(owner, username, name, region_key):
    ...
    # Проверяем ключевые слова региона
    for kw in kws:
        if kw in full:
            return True
    # Для RU/UA/BY/KZ/UZ — кириллица в имени = матч
    if region_key in ("ru", "ua", "by", "kz", "uz"):
        script = detect_script(fname + " " + lname + " " + uname)
        if script == "cyrillic":
            return True
    # Для EN-стран — латиница и нет кириллицы
...
def model_col_kb(names, page=0):
    ...
    for i in range(0, len(chunk), 2):
        idx1 = start + i
        row  = [InlineKeyboardButton(text=chunk[i], callback_data="mdlcol_" + str(idx1))]
        if i + 1 < len(chunk):
            idx2 = start + i + 1
            row.append(InlineKeyboardButton(text=chunk[i+1], callback_data="mdlcol_" + str(idx2)))
        rows.append(row)
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="Назад", callback_data="mdlpage_" + str(page - 1)))
    if end < len(names):
        nav.append(InlineKeyboardButton(text="Далее", callback_data="mdlpage_" + str(page + 1)))
    if nav:
        rows.append(nav)
    rows.append([InlineKeyboardButton(text="Назад в меню", callback_data="search_mode_select")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
async def do_market_search(
    ...
    async def process_item(item, floor):
        ...
        # Режим "по модели" — каждый NFT отдельной карточкой без buckets
        if model_only:
            if not region_match(item["owner"], username, item.get("name"), region):
                return
            if floor and not price_ok_for_floor(price, floor, boost):
                return
            txt = (
                "<b>" + str(title) + " #" + str(num) + "\n"
                "Цена: " + str(price) + " ⭐</b>"
            ...
            if oid:
                cache_owner(oid, item["owner"], username, item.get("name",""), item.get("profile_url"), [item])
        ...
        if has_cat:
            ...
            if floor and not price_ok_for_floor(price, floor, boost):
                return
            if oid:
                if owner_seen.get(oid, 0) >= 3:
                    return
                owner_seen[oid] = owner_seen.get(oid, 0) + 1
            profile_url = item.get("profile_url")
            owner_str   = fmt_owner(item["owner"], username, item.get("name", ""))
            if nft_url2:
                nft_line = "\n<b><a href=\"" + nft_url2 + "\">" + str(title) + " #" + str(num) + "</a></b>"
            else:
                nft_line = "\n<b>" + str(title) + " #" + str(num) + "</b>"
            txt = "<b>Владелец: " + owner_str + "</b>" + nft_line + "\n<b>Цена: " + str(price) + " ⭐</b>"
            if oid:
                cache_owner(oid, item["owner"], username, item.get("name",""), profile_url, [item])
    ...
    try:
        await status_msg.edit_text("<b>Сканирую коллекции...</b>", reply_markup=stop_kb())
        ...
        for gid in total_ids:
            if not is_searching or found >= max_results:
                break

            # Получаем флор если нужна фильтрация по категории
            floor = None
            if has_cat:
            ...
            if now - last_upd > 2.0:
                try:
                    ...
                    await status_msg.edit_text(
                        "<b>Проверено коллекций: " + str(scanned) + "/" + str(len(total_ids)) + "\n"
...
# ===================== CORE: ПРОФИЛИ (гифты в профиле, НЕ на маркете) =====================
async def do_profile_search(
    ...
    is_searching = True
    found        = 0
    seen_owners  = {}           # uid -> (owner_obj, username, name)
    last_upd     = 0.0
    START_TIME   = asyncio.get_event_loop().time()
    HARD_LIMIT   = 120.0

    # Шаг 1: собираем всех владельцев с маркета (чтобы знать у кого сколько лотов)
    async def build_market_index():
        ...
        for gid in ids:
            ...
            for item in items:
                ...
                seen_owners[uid]["market_cnt"] = seen_owners[uid]["market_cnt"] + 1

    # Шаг 2: загружаем гифты из профиля (только NFT, у которых есть slug)
    async def fast_saved_gifts(user_id):
        try:
            result = await tg_client(GetSavedStarGiftsRequest(
                peer=await tg_client.get_input_entity(user_id),
                offset="", limit=100,
            ...
            items = []
            for gift in (getattr(result, "gifts", None) or []):
                # Проверяем флаг unsaved/hidden — пропускаем выставленные на продажу
                # (у них обычно есть resell_amount, но мы не имеем доступа здесь,
                # поэтому считаем всё что в SavedStarGifts — это профиль)
                nft_url = make_nft_url(gift)
                if not nft_url:
                    inner = getattr(gift, "gift", None)
                    if inner:
                ...
                if nft_url:
                    ...
                    title = getattr(inner, "title", None) or getattr(gift, "title", "?")
                    num   = getattr(gift, "num", "?")
                    items.append({"title": title, "num": num, "nft_url": nft_url})
            return items
        except Exception as e:
            logger.debug("fast_saved_gifts uid=%s: %s", user_id, e)
            return []
    ...
    async def check_one(uid, info):
        ...
        # Загружаем гифты из профиля
        profile_gifts = await fast_saved_gifts(uid)
        profile_cnt   = len(profile_gifts)

        # Гифты которые в профиле но не на рынке = profile - market
        hidden_cnt = profile_cnt - market_cnt
        if hidden_cnt <= 0:
            return
        if not gifts_in_range(hidden_cnt, min_gifts, max_gifts):
            return

        profile_url = ("https://t.me/" + username) if username else ("tg://user?id=" + str(uid))
        owner_str   = fmt_owner(owner_obj, username, name)
        cache_owner(uid, owner_obj, username, name, profile_url, profile_gifts)
        kb  = owner_card_kb(username, profile_url, uid)
        txt = (
            "<b>Владелец: " + owner_str + "\n"
            "Гифтов в профиле: " + str(profile_cnt) + "\n"
            "Из них скрыто от рынка: " + str(hidden_cnt) + "</b>"
        )
        try:
            await status_msg.bot.send_message(
                chat_id=status_msg.chat.id, text=txt,
    ...
    try:
        ...
        PBATCH  = 5
        checked = 0
        for i in range(0, len(all_owners), PBATCH):
            if not is_searching or found >= max_results:
                break
            if asyncio.get_event_loop().time() - START_TIME > HARD_LIMIT:
                break
            batch = all_owners[i:i+PBATCH]
            tasks = [check_one(uid, info) for uid, info in batch]
...
async def cb_mkt_who(cb: CallbackQuery, state: FSMContext):
    ...
    girls    = (who == "girls")
    model    = (who == "model")
    await state.update_data(search_type="market", cat=real_cat, girls=girls, model=model, ids=None, col_name=None)
    await cb.message.answer(
        "<b>Сколько результатов показать?</b>",
        parse_mode="HTML",
        reply_markup=limit_picker_kb()
    )
    await state.set_state(SearchLimit.limit)
    await cb.answer()
...
async def cb_prf_who(cb: CallbackQuery, state: FSMContext):
    ...
    girls = (who == "girls")
    model = (who == "model")
    await state.update_data(search_type="profile", girls=girls, model=model, ids=None)
    await cb.message.answer(
        "<b>Сколько результатов показать?</b>",
        parse_mode="HTML",
        reply_markup=limit_picker_kb()
...
# Поиск по модели — показываем список всех коллекций
...
async def cb_mode_model(cb: CallbackQuery):
    ...
    total = len(names)
    await cb.message.answer(
        "<b>Поиск по модели. Выбери коллекцию NFT:\n"
        "Всего коллекций: " + str(total) + "</b>",
        parse_mode="HTML",
        reply_markup=model_col_kb(names, page=0)
    )
    await cb.answer()
...
async def cb_mdlpage(cb: CallbackQuery):
    ...
    await cb.message.edit_text(
        ...
        "Всего коллекций: " + str(total) + "</b>",
        parse_mode="HTML",
        reply_markup=model_col_kb(names, page=page)
    )
    await cb.answer()

# Выбор коллекции для поиска по модели — запускаем маркет с model_only=True
...
async def cb_mdlcol(cb: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(search_type="market", cat=None, girls=False, model=True, ids=[col_id], col_name=col_name)
    await cb.message.answer(
        "<b>Сколько результатов показать?</b>",
        parse_mode="HTML",
        reply_markup=limit_picker_kb()
...
async def cb_limit_pick(cb: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    stype    = data.get("search_type", "market")
    girls    = data.get("girls", False)
    model    = data.get("model", False)
    cat      = data.get("cat")
    ids      = data.get("ids")
    col_name = data.get("col_name")
    await cb.answer("Лимит: " + str(lim))
    if stype == "profile":
        await run_profile(cb, girls=girls, model=model, ids=ids, limit=lim)
    else:
        await run_market(cb, cat=cat, girls=girls, model=model, ids=ids, col_name=col_name, limit=lim)
